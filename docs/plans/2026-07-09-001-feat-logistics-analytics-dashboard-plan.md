...
| Field | Decision |
|---|---|
| Objective | Build and deploy a reviewer-ready web app for a logistics client that combines a KPI dashboard, AI-orchestrated natural-language analytics, dynamic charts, explainability, demand forecasting, and read-only data access over one unified dataset, with optional PostgreSQL runtime support for production-style deployment. |
| Primary source of truth | `mock_logistics_data (1).csv` and `data/logistics.csv` as the canonical seed/reference/runtime fallback dataset, an optional PostgreSQL `logistics_orders` runtime adapter, the assignment prompt, and the local logistics assignment files. |
...
The assignment allows the provided dataset or a database, so the canonical CSV remains a valid read-only runtime source while PostgreSQL is supported as a production-style runtime adapter seeded from the same data.
Product Contract clarification: R23-R25 and AE6 treat PostgreSQL as optional runtime support because the assignment requires a provided dataset or database, not a mandatory database.
...
When PostgreSQL is configured, the `logistics_orders` table must preserve the same row count, status distribution, date range, and numeric precision as the CSV seed.
...
- R23. The app must read logistics data through a typed repository layer that supports the canonical CSV dataset and can switch to PostgreSQL when configured, without creating divergent analytics paths.
- R24. Every reviewer-facing runtime data path must be read-only; if PostgreSQL is enabled, the application database user should have `select` access only and the app must expose no insert, update, delete, or mutation route.
- R25. PostgreSQL-backed dashboard, query, and forecasting results, when enabled, must match the CSV baseline for the canonical dataset profile and core KPI calculations.
...
- AE6. Given the app is deployed with either CSV fallback or PostgreSQL environment variables, when `/api/dashboard` and `/api/ask` run, then they load from the selected repository source, return the same canonical KPIs as the seed dataset, and expose no write capability.
...
- Read-only analytics over the provided logistics dataset, with CSV fallback as a valid runtime path and PostgreSQL as an optional production-style runtime path.
- A checked-in optional database migration for `logistics_orders`, a repeatable CSV import path, and parity checks between the seed CSV and database rows when PostgreSQL is enabled.
...
- Full warehouse-style data modeling, ETL orchestration, PostgreSQL realtime sync, or write/admin UI for order management.
...
- KTD2. Use a typed repository as the runtime data boundary. Keep `data/logistics.csv` as the canonical seed/reference and valid fallback runtime source, while supporting PostgreSQL as an optional production-style adapter seeded from the same CSV and parity-checked when enabled.
...
- KTD9. Keep every app data path read-only. The CSV path is read-only by design; when PostgreSQL is enabled, server route handlers use `DATABASE_URL` server-side only and expose no mutation routes. Prefer a read-only database user for deployed runtime access; use an elevated connection only for the local/CI seed import.
- KTD10. Do not allow model-generated SQL even when PostgreSQL is introduced. AI and fallback parsers produce validated DSL requests, and repository methods translate only whitelisted filters and dimensions into parameterized PostgreSQL queries or deterministic in-process aggregations.
- KTD11. If PostgreSQL is used, model the database with concrete Postgres types and constraints: `text` for labels, `date` for order/delivery dates, `integer` for quantity, `numeric` for money/percent fields, `boolean` for promotions, `order_id` as primary key, and a `status` check constraint for known statuses.
...
flowchart TB
  ...
  DataRepo -. optional production adapter .-> PostgreSQL[(PostgreSQL)]
...
- A PostgreSQL database credentials are optional production-hardening inputs, not a prerequisite for satisfying the assignment's data requirement.
- The deployed app can read from the canonical CSV fallback or from PostgreSQL when `LOGISTICS_DATA_SOURCE=postgres` and credentials are configured; tests may use the CSV fixture or a mocked repository unless a PostgreSQL test database is explicitly configured.
...
| PostgreSQL credentials missing or misconfigured while PostgreSQL mode is forced | The optional database-backed path cannot load data. | Document `LOGISTICS_DATA_SOURCE=csv` fallback, require clear server-side configuration errors for forced PostgreSQL mode, and run smoke checks against the selected data source. |
| Database seed drifts from CSV when PostgreSQL is enabled | KPI results may differ from tested expectations. | Add import parity checks for row count, status counts, max date, and core KPIs before using PostgreSQL for deployment. |
| PostgreSQL table accidentally allows writes when database mode is enabled | Assignment requires read-only data handling. | Use a read-only deployed database user when available, omit mutation routes, and reserve elevated credentials for one-off import tooling only. |
...
- PostgreSQL is an optional runtime adapter for the read-only dataset, with typed schema constraints and environment-variable configuration documented in `README.md`; the canonical CSV fallback remains assignment-valid.
...
- **Verification:** CSV parsing is centralized, deterministic, and reusable by the PostgreSQL import/parity milestone while remaining a valid read-only runtime fallback.

### U2A. Optional PostgreSQL Schema and Import Path
...
- **Dependencies:** U2.
- **Files:** `db/migrations/001_create_logistics_orders.sql`, `scripts/import-logistics-csv.mjs`, `.env.example`, `README.md`, `tests/unit/analytics.test.ts`.
- **Approach:** Create a `public.logistics_orders` table with one row per order. Use `order_id text primary key`, `date` columns for `order_date` and `delivery_date`, `integer` for `quantity`, `numeric` for money and discount values, `boolean` for `is_promo`, and `text` for labels/dimensions. Add a `status` check constraint for `delivered`, `delayed`, `in_transit`, `exception`, and `canceled`. Add minimal indexes for known filter paths, starting with `order_date`, `status`, and common dashboard filters only if query shape justifies them.
- **Execution note:** Use an elevated `DATABASE_URL` only for the local/CI import command if needed; never expose database credentials to client code or route responses.
...
- **Test scenarios:**
  ...
  - Migration uses typed constraints and does not create write policies or app mutation paths.
  ...
  - `.env.example` documents `DATABASE_URL`, optional `POSTGRES_SSL`, and `OPENAI_API_KEY` without values.
- **Verification:** A fresh PostgreSQL database can receive the migration and seed import, then report the same canonical dataset profile as `data/logistics.csv`.

### U2B. Repository Source Selection and PostgreSQL Adapter

- **Goal:** Route dashboard, natural-language analytics, and forecasting through one typed data repository that uses the canonical CSV fallback by default and PostgreSQL when explicitly configured.
- **Requirements:** R1, R5, R8, R18, R23, R24, R25, AE1, AE2, AE3, AE4, AE6.
- **Dependencies:** U2, with U2A required only for the PostgreSQL-backed adapter path.
- **Files:** `src/lib/data/logisticsRepository.ts`, `src/lib/data/postgresLogisticsRepository.ts`, `src/lib/data/csvLogisticsRepository.ts`, `src/lib/data/logistics.ts`, `src/app/api/dashboard/route.ts`, `src/app/api/ask/route.ts`, `tests/unit/data/logisticsRepository.test.ts`, `tests/integration/api/dashboard.test.ts`, `tests/integration/api/ask.test.ts`.
- **Approach:** Define a repository contract for reading rows and dataset metadata with whitelisted filters only. Implement a PostgreSQL adapter using `pg` server-side calls with `DATABASE_URL`; keep all analytics computation in deterministic TypeScript unless a future optimized SQL path is explicitly added and tested. Keep the CSV adapter as a test fixture/fallback path and make data source selection explicit via environment configuration.
...
- **Patterns to follow:** Prefer parameterized PostgreSQL queries, map database rows into the existing domain model at the repository boundary, and expose clear errors when PostgreSQL env vars are absent only when PostgreSQL mode is explicitly selected.
- **Test scenarios:**
  - Repository contract tests pass against the CSV adapter and a mocked PostgreSQL adapter.
  - Dashboard and ask route handlers depend on the repository contract rather than direct CSV reads.
  - Missing PostgreSQL environment variables produce a clear server-side configuration error only when `LOGISTICS_DATA_SOURCE=postgres` is explicitly selected.
  - PostgreSQL adapter applies only whitelisted filters and rejects unsupported filter keys before querying.
  - Core KPIs and assignment example answers match the CSV baseline when the mocked PostgreSQL rows mirror `data/logistics.csv`.
- **Verification:** Production routes have one data path through the repository, and the deployed build can use either CSV fallback or PostgreSQL without code changes.
...
- **Verification:** The dashboard API and AI query path both call this tool for overlapping metrics, and results remain source-equivalent when the repository is backed by PostgreSQL or the CSV fixture.
...
- **Goal:** Document and deploy the application so reviewers can evaluate it through a repository link and public URL, with CSV fallback and optional PostgreSQL configuration both explained.
- **Requirements:** R20, R21, R22, R23, R24, R25.
- **Dependencies:** U1, U2, U2B, U7, U8, with U2A required only when the selected deployment uses PostgreSQL.
- **Files:** `README.md`, `.env.example`, `vercel.json`, `tests/e2e/smoke.spec.ts`.
- **Approach:** Write README sections for setup, CSV fallback, optional PostgreSQL migration/import, environment variables, architecture, data flow, AI approach, assumptions, limitations, future improvements, deployment URL, and test credentials if auth is ever added. Deploy to Vercel using one explicit read-only data source, configure `OPENAI_API_KEY` when available, configure PostgreSQL variables only when the database adapter is selected, and verify the public URL after deployment.
...
- **Test scenarios:** 
  - README setup instructions let a fresh developer run the app locally from CSV fallback and optionally with PostgreSQL configured, with or without `OPENAI_API_KEY`.
  ...
  - PostgreSQL parity verification confirms deployed runtime data matches the CSV seed profile before final submission when PostgreSQL is selected.
...
| Source parity check | U2, U2A, U2B, U9 | CSV dataset profile is locked; when PostgreSQL is enabled, PostgreSQL row count, status counts, max date, and core KPIs match `data/logistics.csv`, and the app role has select-only access. |
...
- The repository contains a working full-stack app, the canonical CSV seed, optional PostgreSQL migration/import path, tests, README, and deployment configuration.
- The deployed runtime uses one selected read-only data source through the repository layer; if PostgreSQL is selected, it is protected as read-only for application access and parity-checked against the seed dataset.
...
- No secrets are committed, and `.env.example` documents optional OpenAI and PostgreSQL configuration.
